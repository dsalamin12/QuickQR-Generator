...
// ── Sticky right col offset ──
function updateStickyTop() {
  const hh = document.getElementById('site-header').offsetHeight;
  const rightCol = document.getElementById('right-col');
  if (rightCol) {
    rightCol.style.top = (hh + 12) + 'px';
    rightCol.style.maxHeight = 'calc(100vh - ' + (hh + 24) + 'px)';
  }
}
updateStickyTop();
...
(function initFeedback() {
  // Load stored votes
  const likeCount = localStorage.getItem('quickqr-likes') || 0;
  const dislikeCount = localStorage.getItem('quickqr-dislikes') || 0;
  const userVote = localStorage.getItem('quickqr-vote'); // 'like', 'dislike', null
  ...
  if (userVote === 'like') likeBtn.classList.add('voted');
  if (userVote === 'dislike') dislikeBtn.classList.add('voted');

  likeBtn.addEventListener('click', () => {
    if (userVote === 'like') return;
    const inc = userVote === 'dislike' ? 1 : 1; // if switching, decrement dislike but we'll simply increment like and clear dislike
    let newDislikes = parseInt(localStorage.getItem('quickqr-dislikes')) || 0;
    if (userVote === 'dislike') {
      newDislikes = Math.max(0, newDislikes - 1);
      localStorage.setItem('quickqr-dislikes', newDislikes);
      document.getElementById('dislike-count').textContent = newDislikes;
      dislikeBtn.classList.remove('voted');
    }
    const newLikes = (parseInt(localStorage.getItem('quickqr-likes')) || 0) + 1;
    localStorage.setItem('quickqr-likes', newLikes);
    localStorage.setItem('quickqr-vote', 'like');
    document.getElementById('like-count').textContent = newLikes;
    likeBtn.classList.add('voted');
    showToast('👍 Thanks for the love!', 'var(--success)');
  });

  dislikeBtn.addEventListener('click', () => {
    if (userVote === 'dislike') return;
    let newLikes = parseInt(localStorage.getItem('quickqr-likes')) || 0;
    if (userVote === 'like') {
    ...
    const newDislikes = (parseInt(localStorage.getItem('quickqr-dislikes')) || 0) + 1;
    localStorage.setItem('quickqr-dislikes', newDislikes);
    localStorage.setItem('quickqr-vote', 'dislike');
    document.getElementById('dislike-count').textContent = newDislikes;
    dislikeBtn.classList.add('voted');
    showToast('👎 Thanks for your feedback, we’ll improve.', 'var(--warning)');
  });

  // Compliment buttons
  document.querySelectorAll('.compliment-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      showToast(`❤️ ${btn.textContent.trim()} — we appreciate it!`, 'var(--success)');
    });
  });
  ...
  document.getElementById('submit-feedback').addEventListener('click', () => {
    ...
    if (!comment) return showToast('✏️ Write a suggestion first!', '#f59e0b');
    // Just show a thank you; no backend
    showToast('✅ Thank you! Your suggestion has been noted (client‑side only).', 'var(--success)');
